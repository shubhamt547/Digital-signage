...
class Index(View):
    def get(self, request):
        ...
        return render(request, 'personal/home.html', context={'asset_form': asset_form, 'active_assets': active_assets,
                                                              'inactive_assets': inactive_assets})

    def post(self, request):
        asset_form = AddAssetForm(request.POST, request.FILES)
        if asset_form.is_valid():
            instance = Asset(source=request.FILES['file'], start_date=timezone.now(),
    ...
    def post(self, request):
        try:
            ...
            return JsonResponse({'success': True,
                                 'message': 'Successfully updated the status of {} to {}'.format(asset_object.name,
                                                                                                 status)})
        except Exception as e:
            print(e)
...
class FileDownload(View):
    def get(self, request):
        try:
            ...
            lst = []
            for i in active_assets:
                lst.append({'duration': i.duration, 'url': i.source.url, 'id': i.id, 'name': i.name})
            return JsonResponse({'success': True, 'asset': lst})
        except Exception as e:
            print(e)
            return JsonResponse({'success': False})
